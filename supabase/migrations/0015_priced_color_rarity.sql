-- 0015 — expose color_identity + rarity in deck_cards_priced, so the deck view
-- can group/sort by color category and rarity (Moxfield-style view options).
-- CREATE OR REPLACE can't change a function's return type, so drop + recreate.
